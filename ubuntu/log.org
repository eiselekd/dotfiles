...
(
 ...
 git checkout master
 sh autogen.sh
 ./configure --with-linux=$d/linuxPATH --with-linux-obj=$d/linux
 make -s -j `getconf _NPROCESSORS_ONLN` deb
)
(
cd ../zfs
git checkout master
sh autogen.sh
./configure --with-spl=$d/spl and --with-spl-obj=$d/spl --with-linux=$d/linux --with-linux-obj=$d/linux
make -s -j `getconf _NPROCESSORS_ONLN` deg
)
