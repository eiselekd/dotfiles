...
(defun etu/test-file-str-in-tags-buffer ()
  ...
  (with-temp-buffer
    ...
    (cl-assert (string= "/home/mk/foo" (etu/file-str-in-tags-buffer (current-buffer) "/home/mk/foo")))
    (cl-assert (string= "bar" (etu/file-str-in-tags-buffer (current-buffer) "bar")))
    (cl-assert (string= "/home/mk/abcdefg" (etu/file-str-in-tags-buffer (current-buffer) "/home/mk/abcdefg")))
    (cl-assert (null (etu/file-str-in-tags-buffer (current-buffer) "/home/mk/abc")))
    (cl-assert (null (etu/file-str-in-tags-buffer (current-buffer) "bcd")))
    (cl-assert (null (etu/file-str-in-tags-buffer (current-buffer) "mk/abcdefg")))
    (cl-assert (null (etu/file-str-in-tags-buffer (current-buffer) "junkline")))))
...
the file is not already in TAGS, maybe add it."
  (interactive)
  (message "[+] run etu/update-tags-for-file for %s : tags-file-name:%s" buffer-file-name tags-file-name)
  ...
  (catch 'etu/update-tags-for-file
    (when tags-file-name
      ...
      (let* ((file              (buffer-file-name (current-buffer)))
        ...
        (message "Refreshing TAGS file for %s... with '%s'" file cmd)
