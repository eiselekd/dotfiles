;;; cling-utils.el --- General utility functions used by cling-mode modules -*- lexical-binding: t -*-
...
;;
;; See also `cling-str.el' for string utility functions.
;;
;; All symbols in this module have a `cling-utils-' prefix.
...
This function supports the SafeCling and PackageImports syntax extensions.
...
(defun cling-utils-async-update-post-command-flag ()
...
`cling-utils-async-post-command-flag'."
  (let* ((cmd this-command)
         (updated-flag (cons cmd cling-utils-async-post-command-flag)))
    (setq cling-utils-async-post-command-flag updated-flag)))

(defun cling-utils-async-watch-changes ()
  "Watch for triggered commands during async operation execution.
Resets flag variable
`cling-utils-async-update-post-command-flag' to NIL.  By changes it is
...
buffer, point was not moved, etc.  To collect data `post-command-hook' is used."
  (setq cling-utils-async-post-command-flag nil)
  (add-hook
   'post-command-hook #'cling-utils-async-update-post-command-flag nil t))

(defun cling-utils-async-stop-watching-changes (buffer)
  "Clean up after async operation finished.
This function takes care about cleaning up things made by
`cling-utils-async-watch-changes'.  The BUFFER argument is a buffer where
...
execusion."
  (with-current-buffer buffer
    (setq cling-utils-async-post-command-flag nil)
    (remove-hook
     'post-command-hook #'cling-utils-async-update-post-command-flag t)))

(defun cling-utils-reduce-string (str)
...
(defun cling-utils-repl-response-error-status (response)
...
(defun cling-utils-compose-type-at-command (pos)
  "Prepare :type-at command to be send to cling process.
...
(defun cling-mode-toggle-interactive-prompt-state (&optional disabled)
  "Set `cling-mode-interactive-prompt-state' to t.
If given DISABLED argument sets variable value to nil, otherwise to t."
  (setq cling-mode-interactive-prompt-state (not disabled)))

(provide 'cling-utils)
;;; cling-utils.el ends here
