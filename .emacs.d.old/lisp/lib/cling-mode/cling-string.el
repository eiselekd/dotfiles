;;; cling-string.el --- Cling related string utilities -*- lexical-binding: t -*-
...
(defun cling-string-trim (string)
  "Remove whitespace around STRING.

A Whitespace character is defined in the Cling Report as follows
...
(defun cling-string-only-spaces-p (string)
  "Return t if STRING contains only whitespace (or is empty)."
  (string= "" (cling-string-trim string)))

(defun cling-string-take (string n)
  "Return (up to) N character length prefix of STRING."
  (substring string 0 (min (length string) n)))

(defconst cling-string-literal-encode-ascii-array
...
(defun cling-string-literal-encode (str &optional no-quotes)
  "Encode STR according Cling escape rules using 7-bit ASCII representation.
...
This is the dual operation to `cling-string-literal-decode'."
                    ...
                    (let ((lc lastc))
                      ...
                      (if (>= c 128) ;; if non-ASCII code point
                        ...
                        (if (or (and (= lc 14) (= c ?H)) ;; "\SO\&H"
                            ...
                            (concat "\\&" (aref cling-string-literal-encode-ascii-array c))
                          (aref cling-string-literal-encode-ascii-array c)
                          ))))))
...
(defconst cling-string-literal-escapes-regexp
  (concat "[\\]\\(?:"
          (regexp-opt (append
...
See Cling Report Sect 2.6,
URL `http://www.cling.org/onlinereport/cling2010/clingch2.html#x7-200002.6',
for more details.")

(defconst cling-string-literal-decode1-table
...
Used by `cling-string-literal-decode1'.")

(defun cling-string-literal-decode1 (l)
  "Decode a single string literal escape sequence.
L must contain exactly one escape sequence.
This is an internal function used by `cling-string-literal-decode'."
  (let ((case-fold-search nil))
    (cond
     ((gethash l cling-string-literal-decode1-table))
...
(defun cling-string-literal-decode (estr &optional no-quotes)
  "Decode a Cling string-literal.
If NO-QUOTES is nil, ESTR must be surrounded by quotes.

This is the dual operation to `cling-string-literal-encode'."
  (if (and (not no-quotes)
    ...
    (let ((s (if no-quotes ;; else: do general decoding
      ...
      (replace-regexp-in-string cling-string-literal-escapes-regexp #'cling-string-literal-decode1 s t t))))

(defun cling-string-ellipsize (string n)
  "Return STRING truncated to (at most) N characters.
If truncation occured, last character in string is replaced by `…'.
See also `cling-string-take'."
  (cond
   ((<= (length string) n) string) ;; no truncation needed
   ((< n 1) "")
   (t (concat (substring string 0 (1- n)) "…"))))

(provide 'cling-string)

;;; cling-string.el ends here
